...
let cards = ['A','K', 'Q', 'T', '9', '8', '7', '6', '5', '4','3','2','1','J'];
...
function compareCards(x,y){
  ...
  for (let i = 0; i < x.length; i++){
    if (card1pass.includes(x[i]) || x[i] === 'J'){
      if ((!duplicates1.includes(x[i])) && x[i] != 'J') duplicates1.push(x[i]);
    }
    else card1pass.push(x[i]);
    if (card2pass.includes(y[i]) || y[i] === 'J'){
      if ((!duplicates2.includes(y[i])) && y[i] != 'J') duplicates2.push(y[i]);
    }
    else card2pass.push(y[i]);
  }
  if (x.includes('J') && duplicates1.length == 0) duplicates1.push('J');
  if (y.includes('J') && duplicates2.length == 0) duplicates2.push('J');
  if (x.includes('J') && card1pass.length == 0) card1pass.push('J');
  if (y.includes('J') && card2pass.length == 0) card2pass.push('J');
